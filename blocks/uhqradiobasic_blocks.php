...
function b_uhqradiobasic_status_show($options)
{
	...
	if ($errno) {
	  $block['status'] = _MH_UHQRADIOBASIC_OFFAIR;
		$block['statusimg'] = _MB_UHQRADIOBASIC_OFFAIR_IMG;
		if ($options[19]) {
			$block['error'] = 1;
	...
	if ($options[2] == "I") {
		...
		if ( strpos($xmlmount,'<artist>') !== FALSE ) {
			...
			if ( strpos(uhqradiobasic_isolatexml($xmlmount,'<title>','</title>'),' - ') ) {
				uhqradiobasic_titlesplit(uhqradiobasic_isolatexml($xmlmount,'<title>','</title>'),
	  ...
	  $block['count'] = uhqradiobasic_isolatexml($xmlmount,'<listeners>','</listeners>');
		...
		if ($status == 1) {
			// Extract Title & Split
			uhqradiobasic_titlesplit( uhqradiobasic_isolatexml($xmlmount,'<SONGTITLE>','</SONGTITLE>'),
			  $block['artist'],$block['title'] );
			// Extract Listener Count
...
function b_uhqradiobasic_status_edit($options)
{
	...
	if ($options[11] == "0") $form .= "checked='checked'";
	$form .= "/>";
	...
	if ($options[13] == "1") $form .= "checked='checked'";
	$form .= "/>";
	$form .= _MB_UHQRADIOBASIC_YES;
	$form .= "<input type='radio' name='options[13]' value= '0' ";
	if ($options[13] == "0") $form .= "checked='checked'";
	$form .= "/>";
	$form .= _MB_UHQRADIOBASIC_NO;
	$form .= "<br />";
	...
	if ($options[18] == "1") $form .= "checked='checked'";
	$form .= "/>";
	$form .= _MB_UHQRADIOBASIC_YES;
	$form .= "<input type='radio' name='options[18]' value= '0' ";
	if ($options[18] == "0") $form .= "checked='checked'";
	$form .= "/>";
	$form .= _MB_UHQRADIOBASIC_NO;
	$form .= "<br />";
	...
	if ($options[19] == "1") $form .= "checked='checked'";
	$form .= "/>";
	$form .= _MB_UHQRADIOBASIC_YES;
	$form .= "<input type='radio' name='options[19]' value= '0' ";
	if ($options[19] == "0") $form .= "checked='checked'";
	$form .= "/>";
	$form .= _MB_UHQRADIOBASIC_NO;
	$form .= "<br />";
